...
//   // 3. Toggle Hide/Show
//   const toggleHideCategory = async (item) => {
//     try {
//       const response = await api({
//         url: SummaryApi.updateCategory.url(item._id),
//         method: SummaryApi.updateCategory.method,
...
//       });
//       if (response.data.success) {
//         toast.success(`Category is now ${!item.showOnWebsite ? 'Visible' : 'Hidden'}`);
//         fetchCategories();
//       }
//     } catch (error) {
...
//   // Handle Action Menu Toggle (Original Logic)
//   const handleToggle = (id) => {
//     if (openMenuId === id) {
//       setOpenMenuId(null);
//     } else {
...
//       // Adjustment if menu goes off screen
//       if (window.innerHeight - rect.bottom < 150) {
//         top = rect.top - 150;
//       }
//       if (window.innerWidth - rect.right < 150) {
...
//   // Outside click listener to close menu
//   useEffect(() => {
//     const handleClickOutside = (event) => {
//       if (
//         menuRef.current &&
...
//               <tr><td colSpan="5" className="p-4 text-center text-gray-400">No matching category found</td></tr>
//             ) : (
//               filteredCategories.concat(filteredCategories).concat(filteredCategories).concat(filteredCategories).concat(filteredCategories).map((item) => (
//                 <tr key={item._id} className="text-sm text-center">
//                   <td className="py-4 px-4 bg-white border-y border-l border-gray-200 rounded-l-lg font-medium">
//                     {item.name}
//                   </td>
//                   <td className="py-4 px-4 bg-white border-y border-gray-200">
//                     <span className={item.status === "Active" ? "text-green-600 font-semibold" : "text-red-500"}>
//                       {item.status}
//                     </span>
//                   </td>
//                   <td className="py-4 px-4 bg-white border-y border-gray-200">
...
//                       <div
//                         ref={menuRef}
//                         className="fixed w-32 bg-white border rounded-lg shadow-lg text-sm z-[9999] overflow-hidden"
//                         style={{
//                           top: menuPosition.top,
//                           left: menuPosition.left,
//                         }}
//                       >
//                         <button
//                           onClick={() => navigate(`/admin-panel/view-category/${item._id}`)}
//                           className="block w-full text-left px-4 py-2 hover:bg-gray-100"
//                         >
//                           View
//                         </button>
//                         <button
//                           onClick={() => navigate(`/admin-panel/edit-category/${item._id}`)}
//                           className="block w-full text-left px-4 py-2 hover:bg-gray-100"
//                         >
//                           Edit
//                         </button>
