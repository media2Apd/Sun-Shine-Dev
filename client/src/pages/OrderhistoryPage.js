...
export default function OrderHistoryPage() {
  ...
  const orders = orderData || []; // fallback to empty array
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 lg:px-8 py-10">
      <h1 className="text-2xl font-semibold text-gray-800 mb-1">Order History</h1>
                ...
                <div className="text-gray-600 text-sm mt-2">
                  ...
                  <ul className="space-y-1">
                    {order.items.map((item, idx) => (
                      <li key={idx}>
                        {item.name} – {item.quantity} {item.stock < 1 && "(Out of Stock)"}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="flex flex-col md:items-end gap-3 mt-4 md:mt-0">
                ...
                <div className="flex flex-col md:flex-row gap-2 mt-2">
                  {order.status === "Delivered" ? (
                    ...
                    <button onClick={()=> navigate('orderdetails-page')}
                     className="bg-green-600 text-white px-6 py-2 rounded-full hover:bg-green-700 transition">
                      Track Order
                    </button>
                  )}
                  <button onClick={()=> navigate('orderdetails-page')} className="border border-gray-300 px-6 py-2 rounded-full text-gray-700 hover:bg-gray-100 transition">
                    Details
                  </button>
                </div>
              </div>
