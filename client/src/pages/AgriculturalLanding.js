...
const AgricultureLanding = () => {
  ...
  const navItems = [
    ...
    { label: 'Why Us', scroll: 'why-us' },
    { label: 'FAQ', path: '/faqs' },
    { label: 'Blogs', path: '/blogs' },
  ];

  // Desktop + Mobile share same navItems — no duplication!

  const handleNav = (item) => {
    if (item.scroll) {
      document.getElementById(item.scroll)
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden font-sans text-slate-900">
      ...
      ${visionHovered ? "text-white/75" : "text-[#00B207]"}`}>
              ...
              <h1 className={`text-2xl md:text-3xl font-bold mb-4 transition-colors duration-700
      ${visionHovered ? "text-white" : "text-[#00B207]"}`}>
                A Greener, More Productive Future
              </h1>
              <p className={`leading-relaxed text-sm md:text-base transition-colors duration-700
      ${visionHovered ? "text-white/80" : "text-gray-600"}`}>
                To become a trusted agricultural brand that supports sustainable farming and improves farmer productivity.
              </p>
            </div>

            {/* Mission — vision hover ஆனா white ஆகும் */}
            <div
              className={`flex-1 p-6 sm:p-10 md:p-14 transition-all duration-700 ease-in-out cursor-default
      ...
      ${visionHovered ? "text-[#009900]" : "text-white/75"}`}>
              ...
              <h1 className={`text-2xl md:text-3xl font-bold mb-4 transition-colors duration-700
      ${visionHovered ? "text-[#00B207]" : "text-white"}`}>
                Delivering Quality, Every Harvest
              </h1>
              <p className={`leading-relaxed text-sm md:text-base transition-colors duration-700
      ${visionHovered ? "text-[#338833]" : "text-white/80"}`}>
