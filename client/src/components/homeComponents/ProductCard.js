...
//         className="absolute top-2 right-2 w-10 h-10 bg-white  rounded-lg flex items-center justify-center shadow-sm"
//       >
//         {loadingWishlist ? (
...
const ProductCard = ({ item }) => {
  ...
  useEffect(() => {
    if (wishlist && wishlist.length > 0) {
      setIsLiked(wishlist.some((i) => i.productId === item._id));
    } else {
      const local = getLocalWishlist();
  ...
  return (
    <div
      onClick={handleCardClick}
      className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition cursor-pointer"
    >
      {/* IMAGE */}
