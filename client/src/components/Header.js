...
//         {/* Logo */}
//         <div>
//           <img onClick={()=> navigate('/')} src={logo} alt="Sunshine Logo" className="h-12 md:h-14 lg:h-16 w-auto object-contain cursor-pointer" />
//         </div>
...
import {
  ...
  FiRefreshCw, FiLogOut, FiUserCheck,
} from "react-icons/fi";
import logo from "../assets/logo.svg";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { useSelector } from 'react-redux';
...
export default function Header() {
  ...
  return (
    // ✅ py-3 — Navbar-ஓட py-3-க்கு match
    // ✅ bg-white/95 backdrop-blur-md — Navbar style-க்கு match
    <header className="bg-white/95 backdrop-blur-md shadow-sm sticky top-0 z-50 font-sans border-b border-gray-100">
      <div className="container mx-auto px-6 py-3 flex items-center justify-between relative">
        
        {/* Logo — ✅ h-8 md:h-10 lg:h-12 Navbar-ஓட logo-க்கு exact match */}
        <div>
          <img
            onClick={() => navigate('/')}
            src={logo}
        ...
        <div className="flex items-center space-x-5 text-gray-700 relative">
          ...
          <div className="hidden md:flex items-center flex-1 max-w-xl mx-6">
            <div className="relative w-full">
              <input
                type="text"
                placeholder="Search products..."
                className="w-full border rounded-md py-1.5 px-4 pr-10 focus:outline-none focus:border-[#00B207] text-sm"
                value={search}
                onChange={handleSearch}
              />
              <FiSearch className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500" />
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {user && (
              <div
                className={`absolute -right-24 top-8 mt-2 w-64 bg-white border border-[#E6E6E6] rounded-md overflow-y-auto max-h-[80vh] z-50 transform ${
                  open
                    ? "opacity-100 translate-y-0 transition-all duration-300 ease-out"
                    : "opacity-0 -translate-y-5 pointer-events-none transition-none"
                }`}
          ...
          {/* Wishlist */}
          <Link to="/wishlist" className="relative">
            <FiHeart strokeWidth={1.5} className="text-xl cursor-pointer hover:text-[#00B207] transition" />
            {wishlistCount > 0 && (
              <span className="absolute -top-1 -right-2 bg-[#FF3B30] text-white text-[10px] w-4 h-4 flex items-center justify-center rounded-full">
                {wishlistCount}
              </span>
          ...
          {/* Cart */}
          <Link to="/cart" className="relative">
            <FiShoppingCart strokeWidth={1.5} className="text-xl cursor-pointer hover:text-[#00B207] transition" />
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-2 bg-[#FF3B30] text-white text-[10px] w-4 h-4 flex items-center justify-center rounded-full">
                {cartCount}
              </span>
